    ...
    return (
      <Form className="addStageForm" dataSet={addStepDs}>
        <Select
          name="type"
          addonAfter={
            <Tips helpText="CI阶段中支持添加构建、发布Chart、代码检查以及自定义类型的CI任务；CD阶段中支持添加部署、主机部署以及人工卡点的CD任务。且流水线中任何CD阶段后，不能再添加CI阶段" />
          }
          onOption={({ record }) =>
            optType !== "edit"
              ? {
        ...
        <Select
          name="parallel"
          addonAfter={
            <Tips helpText="目前CI阶段中的任务只支持并行，CD中的阶段仅支持串行" />
          }
        >
        ...
        {addStepDs?.current?.get("triggerType") === "manual" ? (
          <Select
            ...
            renderer={({ text, value }) => {
              return (
                <Tooltip title={`${text}(${value.loginName})`}>{text}</Tooltip>
              );
            }}
            maxTagCount={3}
            maxTagPlaceholder={(omittedValues) => {
              let tempArr = [];
              forEach(omittedValues, ({ realName, loginName }) => {
                tempArr.push(`${realName}(${loginName})`);
              });
              return (
            ...
            addonAfter={
              <Tips helpText="此处的人工审核默认为”或签“的方式，若选择的审核人员为多个，那么其中一个审核通过，便会开始执行下一阶段。" />
            }
          />
        ) : (
          ""
