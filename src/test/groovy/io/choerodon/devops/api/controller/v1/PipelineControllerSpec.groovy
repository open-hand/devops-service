...
import io.choerodon.devops.api.vo.CheckAuditVO
import io.choerodon.devops.api.vo.PipelineCheckDeployVO
import io.choerodon.devops.api.vo.PipelineRecordListVO
import io.choerodon.devops.api.vo.PipelineRecordReqVO
import io.choerodon.devops.api.vo.PipelineReqVO
...
import io.choerodon.devops.app.service.impl.IamServiceImpl
...
class PipelineControllerSpec extends Specification {
    ...
    IamService iamService = new IamServiceImpl(baseServiceClient)
    @Shared
    ...
    Integer isEnabled = 1
    def setup(){
        DependencyInjectUtil.setAttribute(pipelineService,"iamService",iamService)
        ...
        Mockito.doReturn(projectDO).when(baseServiceClient).queryIamProject(1L)

        List<ProjectWithRoleVO> list = new ArrayList<>()
        PageInfo<ProjectWithRoleVO> pageInfo = new PageInfo(list)
        ResponseEntity<PageInfo<ProjectWithRoleVO>> pageInfoResponseEntity = new ResponseEntity<>(pageInfo, HttpStatus.OK)
        Mockito.when(baseServiceClient.listProjectWithRole(1L,0,0)).thenReturn(pageInfoResponseEntity)
    }
    def "create"(){
        when:
