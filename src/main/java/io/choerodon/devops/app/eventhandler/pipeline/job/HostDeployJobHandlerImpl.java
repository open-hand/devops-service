...
public class HostDeployJobHandlerImpl extends AbstractJobHandler {
    ...
    protected void checkConfigInfo(Long projectId, DevopsCiJobVO devopsCiJobVO) {
        ...
        // 主机id不能为空
        Long hostId = devopsCiHostDeployInfoVO.getHostId();
        if (hostId == null) {
        ...
        if (DeployTypeEnum.CREATE.value().equals(devopsCiHostDeployInfoVO.getDeployType())) {
            // 校验应用编码和应用名称
            devopsHostAppService.checkNameAndCodeUniqueAndThrow(projectId, hostId, null, devopsCiHostDeployInfoVO.getAppName(), devopsCiHostDeployInfoVO.getAppCode());
    ...
    public void fillJobConfigInfo(DevopsCiJobVO devopsCiJobVO) {
        ...
        if (StringUtils.equals(devopsCiHostDeployInfoVO.getHostDeployType(), RdupmTypeEnum.DOCKER_COMPOSE.value())) {
            ...
            Long appId = devopsCiHostDeployInfoVO.getAppId();
            DevopsHostAppDTO devopsHostAppDTO = devopsHostAppService.baseQuery(appId);
            devopsCiHostDeployInfoVO.setValue(dockerComposeValueService.baseQuery(devopsHostAppDTO.getEffectValueId()).getValue());
        } else {
