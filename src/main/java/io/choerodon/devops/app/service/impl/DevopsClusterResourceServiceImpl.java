...
public class DevopsClusterResourceServiceImpl implements DevopsClusterResourceService {
    ...
    private void setPvStatus(DevopsPvDTO devopsPvDTO, String type, DevopsPrometheusVO devopsPrometheusVO) {
        ...
        // Prometheus绑定的PV会出现三种可能状态，1、Available 2、Bound 3、Release
        // Available状态是PV指定给了Prometheus，但是还没有被使用，该情况下可以继续使用
        // Bound    状态是PV指定给了Prometheus，并且Prometheus创建的PVC也绑定了该PV，也可以继续使用
        // Release  状态是PV指定给了Prometheus，并且在安装过程中，Prometheus创建的PVC也绑定了该PV，但是安装过程出现错误，PVC被删除，该PV也就不可继续使用，需要重新选择PV

        // 如果状态是Release，则不设置PvName，前端会根据PvName字段判断是否重新选择PV，为空则重新选择，反之则禁止选择
        if ("Release".equals(devopsPvDTO.getStatus())) {
            return;
        }
