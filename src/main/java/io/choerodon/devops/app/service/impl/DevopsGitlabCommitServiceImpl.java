            ...
            pushWebHookVO.getCommits().forEach(commitDTO -> {
                ...
                if (devopsGitlabCommitDTO == null) {
                    ...
                    if (!CollectionUtils.isEmpty(devopsBranchDTO.getIssueIds())) {
                        devopsIssueRelService.addRelation(DevopsIssueRelObjectTypeEnum.COMMIT.getValue(), devopsGitlabCommitDTO.getId(), devopsBranchDTO.getId(), devopsBranchDTO.getIssueIds());
                    }
                }
            });
        } else {
