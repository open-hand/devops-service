...
@Service
public class HandlerV1IngressRelationsServiceImpl implements HandlerObjectFileRelationsService<V1Ingress> {
    ...
    public void handlerRelations(Map<String, String> objectPath, List<DevopsEnvFileResourceDTO> beforeSync, List<V1Ingress> v1Ingresses, List<V1Endpoints> v1Endpoints, Long envId, Long projectId, String path, Long userId) {
        ...
        if (!devopsIngressService.operateForOldTypeIngressJudgeByClusterVersion(devopsEnvironmentDTO.getClusterId())) {
            ...
            GitOpsUtil.pickCUDResource(beforeIngress, v1Ingresses, addV1Ingress, updateV1Ingress, v1Ingress -> v1Ingress.getMetadata().getName());
    ...
    private void addIngress(Map<String, String> objectPath, Long envId, Long projectId, List<V1Ingress> addV1Ingress, String path, Long userId) {
        addV1Ingress
                .forEach(v1Ingress -> {
                    String filePath = "";
                    try {
                        filePath = objectPath.get(TypeUtil.objToString(v1Ingress.hashCode()));

                        checkIngressAppVersion(v1Ingress);
                        DevopsIngressDTO devopsIngressDTO = devopsIngressService
                                .baseCheckByEnvAndName(envId, v1Ingress.getMetadata().getName());
                        ...
                        devopsEnvFileResourceService.updateOrCreateFileResource(objectPath, envId, null, v1Ingress.hashCode(), devopsIngressDTO.getId(),
                                v1Ingress.getKind());
                ...
                .forEach(v1Ingress -> {
                    ...
                    try {
                        ...
                        if (!isNotChange) {
                            ...
                            DevopsIngressDTO newdevopsIngressDTO = devopsIngressService.baseCheckByEnvAndName(envId, v1Ingress.getMetadata().getName());
                        ...
                        DevopsEnvFileResourceDTO devopsEnvFileResourceDTO = devopsEnvFileResourceService
                                .baseQueryByEnvIdAndResourceId(envId, devopsIngressDTO.getId(), v1Ingress.getKind());
                        devopsEnvFileResourceService.updateOrCreateFileResource(objectPath,
                                envId,
                                devopsEnvFileResourceDTO,
                                v1Ingress.hashCode(), devopsIngressDTO.getId(), v1Ingress.getKind());
    ...
    private void checkIngressAppVersion(
            V1Ingress v1Ingress) {
        try {
            DevopsIngressValidator.checkIngressName(v1Ingress.getMetadata().getName());
        } catch (Exception e) {
            throw new CommonException(e.getMessage(), e);
        }
    }

    private DevopsIngressVO getDevopsIngressDTO(V1Ingress v1Ingress,
                                                Long envId, String filePath) {
        DevopsIngressVO devopsIngressVO = new DevopsIngressVO();
        devopsIngressVO.setDomain(v1Ingress.getSpec().getRules().get(0).getHost()
        );
        devopsIngressVO.setName(v1Ingress.getMetadata().getName());
        devopsIngressVO.setAnnotations(v1Ingress.getMetadata().getAnnotations());
        ...
        for (V1HTTPIngressPath v1HTTPIngressPath : paths) {
            ...
            V1ServiceBackendPort v1ServiceBackendPort = v1IngressServiceBackend.getPort();
            ...
            Integer backendServicePort = v1ServiceBackendPort.getNumber();
            if (PATTERN.matcher(TypeUtil.objToString(backendServicePort)).matches()) {
