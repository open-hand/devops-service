    ...
    public void fileResourceSync(PushWebHookVO pushWebHookVO) {
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(c7nHelmReleaseWithoutPullSecrets)) {
                boolean releaseToSetImagePullSecretsExists = appServiceInstanceService.setImagePullSecrets(pushWebHookVO.getUserId(), devopsEnvironmentDTO, c7nHelmReleaseWithoutPullSecrets, path, devopsEnvCommitDTO.getCommitSha());
                if (releaseToSetImagePullSecretsExists) {
                    return;
                }
            }
