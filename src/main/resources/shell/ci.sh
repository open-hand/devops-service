...
function triggerCdPipeline() {
  http_status_code=$(curl -X POST -o -s -m 10 --connect-timeout 10 -w %{http_code} "${CHOERODON_URL}/v1/cd_pipeline/trigger_cd_pipeline?token=${Token}&commit=${CI_COMMIT_SHA}&ref=${CI_COMMIT_REF_NAME}&gitlab_user_name=${GITLAB_USER_NAME}&gitlab_pipeline_id=${CI_PIPELINE_ID}'")
